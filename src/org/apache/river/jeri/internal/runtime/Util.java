...
public class Util
{
    ...
    private static final String prohibitedProxyInterfacesResource = "org/apache/river/proxy/resources/"
            + "InvocationHandler.moreProhibitedProxyInterfaces";

    /** names of interfaces that proxies are prohibited from implementing */
    private static final Collection prohibitedProxyInterfaces = getProhibitedProxyInterfaces();

    /**
     * Appends the current thread's stack trace to the stack trace of the given exception. This method is used for
     * exceptions that have been unmarshalled as the exceptional result of a remote method invocation, so that the
     * client-side stack trace gets recorded in the exception (while preserving the server-side stack trace in the
     * exception as well). Note that the (somewhat odd) names of this method and the method that this one internally
     * delegates to are significant because these methods visually highlight the remote call boundary between the
     * server-side and the client-side portions to readers of the combined stack trace.
     */
    ...
    /**
     * Clear the stack trace of the given exception by replacing it with an empty StackTraceElement array, and do the
     * same for all of its chained causative exceptions. This method is used when it is desired for the stack trace data
     * of an exception to be suppressed before the exception gets marshalled to a remote virtual machine, perhaps for
    ...
    /**
     * Marshals <code>value</code> to an <code>ObjectOutput</code> stream, <code>out</code>, using RMI's serialization
     * format for arguments or return values. For primitive types, the primitive type's class should be specified (i.e.,
     * for the type <code>int</code>, specify <code>int.class</code>), and the primitive value should be wrapped in
     * instances of the appropriate wrapper class, such as <code>java.lang.Integer</code> or
     * <code>java.lang.Boolean</code>.
     *
     * @param type
     *              <code>Class</code> object for the value to be marshalled
     * @param value
     *              value to marshal
     * @param out
     *              stream to which the value is marshalled
     * @throws IOException
     *                     if an I/O error occurs marshalling the value to the output stream
     */
    ...
    /**
     * Unmarshals a value of the specified <code>type</code> from the <code>ObjectInput</code> stream, <code>in</code>,
     * using RMI's serialization format for arguments or return values and returns the result. For primitive types, the
     * primitive type's class should be specified (i.e., for the primitive type <code>int</code>, specify
     * <code>int.class</code>).
     *
     * @param type
     *             <code>Class</code> object for the value to be unmarshalled
     * @param in
     ...
     *                                if the <code>type</code>'s class could not be found
     */
    ...
    /**
     * Computes the "method hash" of a remote method, <code>m</code>. The method hash is a <code>long</code> containing
     * the first 64 bits of the SHA digest from the UTF encoded string of the method name followed by its "method
     * descriptor". See section 4.3.3 of The Java(TM) Virtual Machine Specification for the definition of a "method
     * descriptor".
     *
     * @param m
     *          remote method
    ...
    /**
     * Returns the method hash for the method <code>m</code>. Subsequent calls to <code>getMethodHash</code> passing the
     * same method argument should be faster since this method caches internally the result of the method to method hash
     * mapping. The method hash is calculated using the <code>computeMethodHash</code> method.
     *
     * @param m
     *          the remote method
    ...
    private static class TableCache extends WeakHashMap
    {
        ...
        public Map getTable(Class remoteClass)
        {
            ...
            /*
             * Method tables for remote classes are cached in a hash table using weak references to hold the Class
             * object keys, so that the cache does not prevent the class from being unloaded, and using soft references
             * to hold the values, so that the computed method tables will generally persist while no objects of the
             * remote class are exported, but their storage may be reclaimed if necessary, and accidental reachability
             * of the remote class through its interfaces is avoided.
             */
            synchronized (this)
            {
                /*
            ...
            /*
             * Check cached reference to method table for this class; if it is null, go and create the table.
             */
            synchronized (tableRef)
            {
                ...
                if (table == null)
                {
                    ...
                    {
                        /*
                         * REMIND: if we hand out this table directly, via a public API, we need to make this map
                         * "unmodifiable".
                         */
    ...
    /**
     * Verifies that the supplied method has at least one declared exception type that is RemoteException or one of its
     * superclasses. If not, then this method throws IllegalArgumentException.
     *
    ...
    /*
     * The following static methods are related to the creation of the "method table" for a remote class, which maps
     * method hashes to the appropriate Method objects for the class's remote methods.
     */

    /**
     * Returns a string consisting of the given method's name followed by its "method descriptor", as appropriate for
     * use in the computation of the "method hash". See section 4.3.3 of The Java(TM) Virtual Machine Specification for
    ...
    /**
     * Returns the descriptor of a particular type, as appropriate for either a parameter type or return type in a
     * method descriptor.
     */
    private static String getTypeDescriptor(Class type)
    {
        ...
        {
            /*
             * According to JLS 20.3.2, the getName() method on Class does return the virtual machine type descriptor
             * format for array classes (only); using that should be quicker than the otherwise obvious code: return "["
             * + getTypeDescriptor(type.getComponentType());
             */
                                            ...
                                            Class cl)
    {
        ...
        for (int i = 0; i < interfaces.length; i++)
        {
            ...
            /*
             * If it is a remote interface (if it extends from java.rmi.Remote) and is not already in the list, then add
             * the interface to the list.
             */
    ...
    /**
     * Throws IllegalArgumentException if any superinterface of c declares a method with the same name and parameter
     * types as m that does not declare RemoteException or a superclass in its throws clause, or if any superinterface
     * of c has its name in prohibitedProxyInterfaces.
     */
    ...
    /**
     * Throws IllegalArgumentException if any superinterface of c declares a method with the given name and parameter
     * types that does not declare RemoteException or a superclass in its throws clause.
     */
    ...
    private static Collection getProhibitedProxyInterfaces()
    {
        ...
        catch (IOException e)
        {
            throw new ExceptionInInitializerError(
                    new IOException("problem getting resources: " + prohibitedProxyInterfacesResource).initCause(e));
        }
            ...
            try
            {
                ...
                try
                {
                    ...
                    while (true)
                    {
                        ...
                        {
                            char c = s.charAt(i);
                            if (prev == '.' ? !Character.isJavaIdentifierStart(c)
                                    : !(Character.isJavaIdentifierPart(c) || (c == '.' && i < n - 1)))
                            {
    ...
    /**
     * Returns the binary name of the given type without package qualification. Nested types are treated no differently
     * from top-level types, so for a nested type, the returned name will still be qualified with the simple name of its
     * enclosing top-level type (and perhaps other enclosing types), the separator will be '$', etc.
     **/
    ...
    /**
     * Returns true either if both arguments are null or if an invocation of Object.equals on "subject" with "object" as
     * the argument returns true; returns false otherwise;
     **/
    ...
    /**
     * Returns true either if both arguments are null or if both arguments refer to objects of the same class and an
     * invocation of Object.equals on "subject" with "object" as the argument returns true; returns false otherwise.
     * This method is used to compare to possibly-null references for object equality when neither object's class is
     * trusted, with the restriction that only objects of the same class can be considered equal.
     **/
    public static boolean sameClassAndEquals(Object subject,
                                             Object object)
    {
        return subject == null ? object == null
                : object != null && subject.getClass() == object.getClass() && subject.equals(object);
    }

    /**
     * Returns true either if both arguments are null of if "subject" is an instance of TrustEquivalence and an
     * invocation of TrustEquivalence.checkTrustEquivalence on "subject" with "object" as the argument returns true;
    ...
    {
        return subject == null ? object == null
                : subject instanceof TrustEquivalence && ((TrustEquivalence) subject).checkTrustEquivalence(object);
    }

    /**
     * Returns true if proxy2 is a generated Proxy (proxy1 is assumed to be one) and the classes of both proxies
     * implement the same ordered list of interfaces, and returns false otherwise.
     */
    public static boolean sameProxyClass(Object proxy1,
                                         Object proxy2)
    {
        return (proxy1.getClass() == proxy2.getClass()
                || (Proxy.isProxyClass(proxy2.getClass()) && equalInterfaces(proxy1, proxy2)));
    }
    ...
    private static class ClientSubjectImpl implements ClientSubject
    {
        ...
        private static final Permission getClientSubjectPerm = new ContextPermission(
     ...
     *                           if a security manager exists and invoking its <code>checkPackageAccess</code> method
     *                           with the package name of the specified class throws a <code>SecurityException</code>
     */
