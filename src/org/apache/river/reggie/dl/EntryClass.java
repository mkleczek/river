 ...
 * and the server. Note that we don't transmit information about interfaces implemented by the class, because it isn't
 * necessary given the specific use of type information for entries.
 * <p>
...
public class EntryClass implements Serializable
{
    ...
    /**
     * An instance containing only name and hash, no superclass info. This is only used on the registrar side, to
     * minimize the amount of info transmitted back to clients.
     */
    ...
    public Class toClass(String codebase) throws IOException, ClassNotFoundException
    {
        ...
        {
            throw new UnmarshalException("problem obtaining local version of " + toString(), e);
        }
        if (hash != local.hash)
            throw new UnmarshalException("incoming entry type: " + toString() + " is not assignable to the local"
    ...
    /**
     * Returns true if the object passed in is an instance of EntryClass with the same type hash as this object. Returns
     * false otherwise.
     * 
     * @param o
    ...
    /**
     * Computes a SHA-1 digest from the hash of the superclass, if there is a superclass, followed by the name of this
     * class, followed by the name and type for each field, if any, declared by this class and ordered alphabetically by
     * field name. The first 8 bytes of the digest are used to form the 64-bit hash value for this type.
     */
    ...
    /**
     * Samples integrity protection setting (if any) of the stream from which this instance is being deserialized and
     * checks that valid values for this object have been read from the stream.
     */
