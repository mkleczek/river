...
/**
 * An EntryRep contains the fields of an Entry packaged up for transmission between client-side proxies and the
...
public class EntryRep implements Serializable, Cloneable
{
    ...
    /**
     * Convert back to an Entry. If the Entry cannot be constructed, null is returned. If a field cannot be
     * unmarshalled, it is set to null.
     */
    public Entry get()
    {
        try
                ...
                catch (Throwable e)
                {
                    if (e instanceof IllegalArgumentException)
                    {
                        ...
                        String msg = "unable to assign "
                                + ((val != null) ? "value of type " + val.getClass().getName() : "null") + " to field "
                                + rf.getDeclaringClass().getName() + "." + rf.getName() + " of type "
                                + rf.getType().getName();
    ...
    /**
     * We don't need this in the client or the server, but since we redefine equals we provide a minimal hashCode that
     * works.
     */
    public int hashCode()
    {
    ...
    /**
     * EntryReps are equal if they have the same class and the fields are pairwise equal. This is really only needed in
     * the server, but it's very convenient to have here.
     */
    public boolean equals(Object obj)
    {
        if (obj instanceof EntryRep)
        {
            ...
            if (!eclass.equals(entry.eclass) || fields.length != entry.fields.length)
                return false;
    ...
    /**
     * Deep clone (which just means cloning the fields array too). This is really only needed in the server, but it's
     * very convenient to have here.
     */
    public Object clone()
    {
        try
    ...
    /**
     * Converts an array of Entry to an array of EntryRep. If needCodebase is false, then the codebase of every EntryRep
     * will be null.
     */
