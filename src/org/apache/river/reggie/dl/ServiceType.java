 ...
 * @see ClassMapper see ClassResolver (org.apache.river.reggie.test.share.ClassResolver?)
 */
public class ServiceType implements Serializable
{
    ...
    /**
     * Class name. If the class is generated by java.lang.reflect.Proxy, then the name is of the form
    ...
    /**
     * Descriptor for the interfaces. As a special case, interfaces is null for the descriptor for java.lang.Object, and
     * non-null otherwise. This avoids carrying a boolean isInterface around.
     *
     * @serial
     */
    protected ServiceType[] interfaces;
    /**
     * An instance containing only name, no supertype info. This is only used on the registrar side, to minimize the
     * amount of info transmitted back to clients.
     */
    ...
    private boolean isAssignableFrom(ServiceType[] ifaces)
    {
        ...
        {
            if (hash == ifaces[i].hash || isAssignableFrom(ifaces[i].interfaces))
                return true;
        }
        return false;
    }
    ...
    /**
     * Returns true if this type is equal to <code>type</code> or if this type is equal to a superclass of
     * <code>type</code>.
     *
     * @param cls
    ...
    public boolean isAssignableFrom(ServiceType cls)
    {
        ...
        if (isInterface())
        {
            if (cls.interfaces != null && isAssignableFrom(cls.interfaces))
                return true;
            for (ServiceType sup = cls.superclass; sup != null && sup.interfaces != null; sup = sup.superclass)
            {
        ...
        {
            for (ServiceType sup = cls.superclass; sup != null; sup = sup.superclass)
            {
                if (hash == sup.hash)
                    return true;
            }
    ...
    private void readObject(ObjectInputStream in) throws IOException, ClassNotFoundException
    {
        ...
        if (hash == 0)
        {
            ...
            {
                throw new UnmarshalException("unable to calculate the type" + " hash for " + name, e);
            }
        }
    }
