 ...
 * <p>
 * The declared methods are pretty straightforward transformations of the ServiceRegistrar and ServiceRegistration
 * interfaces, with external classes (ServiceItem, ServiceTemplate, ServiceMatches, Entry) converted to internal classes
 * (Item, Template, Matches, EntryRep). In addition, there are methods for transformed Lease and LeaseMap interfaces,
 * for service and event leases.
 *
...
public interface Registrar extends Remote, ServiceProxyAccessor, Administrable, DiscoveryAdmin, JoinAdmin, DestroyAdmin
{
    ...
    /**
     * Returns the service object (i.e., just ServiceItem.service) from an item matching the template, or null if there
     * is no match.
     * 
    ...
    /**
     * Looks at all service items that match the specified template, finds every entry (among those service items) that
     * either doesn't match any entry templates or is a subclass of at least one matching entry template, and returns
     * the set of the (most specific) classes of those entries.
     * 
    ...
    /**
     * Looks at all service items that match the specified template, finds every entry (among those service items) that
     * matches tmpl.attributeSetTemplates[setIndex], and returns the set of values of the specified field of those
     * entries. The field name has been converted to an index (fields numbered from super to subclass).
     *
    ...
    /**
     * Looks at all service items that match the specified template, and for every service item finds the most specific
     * type (class or interface) or types the service item is an instance of that are neither equal to, nor a superclass
     * of, any of the service types in the template and that have names that start with the specified prefix, and
     * returns the set of all such types.
     * 
    ...
    /**
     * Adds the specified attribute sets (those that aren't duplicates of existing attribute sets) to the registered
     * service item.
     * 
