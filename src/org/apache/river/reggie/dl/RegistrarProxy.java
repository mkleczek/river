...
/**
 * A RegistrarProxy is a proxy for a registrar. Clients only see instances via the ServiceRegistrar, Administrable and
...
public class RegistrarProxy implements ServiceRegistrar, Administrable, ReferentUuid, Serializable
{
    ...
    private static final Logger logger = Logger.getLogger("org.apache.river.reggie");
    ...
    /**
     * Returns RegistrarProxy or ConstrainableRegistrarProxy instance, depending on whether given server implements
     * RemoteMethodControl.
     */
    public static RegistrarProxy getInstance(Registrar server,
                                             ServiceID registrarID)
    {
        return (server instanceof RemoteMethodControl) ? new ConstrainableRegistrarProxy(server, registrarID, null)
    ...
    public EventRegistration notify(ServiceTemplate tmpl,
            ...
            throws RemoteException
    {
        return server.notify(new Template(tmpl), transitions, listener, handback, leaseDuration);
    }

    // Inherit javadoc
    public Class[] getEntryClasses(ServiceTemplate tmpl) throws RemoteException
    {
        return EntryClassBase.toClass(server.getEntryClasses(new Template(tmpl)));
    }
            ...
            throws NoSuchFieldException, RemoteException
    {
        ...
        Object[] values = server.getFieldValues(new Template(tmpl), setIndex, fidx);
     ...
     * Otherwise print the exception stack trace if debugging is enabled.
     */
    static void handleException(final Throwable e)
    {
        if (e instanceof Error
                && !(e instanceof LinkageError || e instanceof OutOfMemoryError || e instanceof StackOverflowError))
        {
            throw (Error) e;
        }
    ...
    public Class[] getServiceTypes(ServiceTemplate tmpl,
            ...
            throws RemoteException
    {
        return ServiceTypeBase.toClass(server.getServiceTypes(new Template(tmpl), prefix));
    }
    ...
    {
        return UuidFactory.create(registrarID.getMostSignificantBits(), registrarID.getLeastSignificantBits());
    }
    ...
    /**
     * Returns a string created from the proxy class name, the registrar's service ID, and the result of the underlying
     * proxy's toString method.
     *
     * @return String
     */
    public String toString()
    {
        return this.getClass().getName() + "[registrar=" + registrarID + " " + server + "]";
    }
     ...
     * specified by the ServiceID.writeBytes method. Verifies that the deserialized registrar reference is non-null.
     */
